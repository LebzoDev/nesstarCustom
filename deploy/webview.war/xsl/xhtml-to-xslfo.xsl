...
<xsl:stylesheet
   ...
   <!--Parameter sent from Java code.-->
   <xsl:param name="baseurl"/>
   <!-- If no value is sent as parameter, default to 1.5  -->
  ...
  =============================================== -->
   ...
   <xsl:template match="after">
    ...
    <!-- ============================================
    Inserting the already formatted page sequence here.
   =============================================== -->

   <xsl:template match="pageNumberSequence">
  ...
  =============================================== -->
   ...
   <xsl:template match="a">
    ...
    <!-- =====================================
     The header tag, displays a logo on the front page
   ======================================= -->
   <xsl:template match="header">
     <fo:external-graphic src="{@href}" content-width="{@width}" />
   </xsl:template>
  ...
  =============================================== -->
   ...
   <xsl:template match="h3">
      <fo:block font-size="18pt" line-height="21pt"
                keep-with-next="always" space-before="2pt" space-after="2pt">
         <xsl:attribute name="id">
            <xsl:choose>
               <xsl:when test="@id">
                  <xsl:value-of select="@id"/>
  ...
  =============================================== -->
   ...
   <xsl:template match="h5">
      <fo:block font-size="12pt" line-height="14pt"
                keep-with-next="always" space-after="1pt"
                space-before="3pt">
         <xsl:attribute name="id">
            <xsl:choose>
               <xsl:when test="@id">
  ...
  =============================================== -->
   ...
   <xsl:template match="div">
      <fo:block space-after="2pt">
         <xsl:attribute name="space-before">
          ...
          <xsl:choose>
            <xsl:when test="@class='heading' or @class='startheading'">
               <xsl:attribute name="font-size">
                  <xsl:text>14pt</xsl:text>
               </xsl:attribute>
               <xsl:attribute name="font-weight">
   ...
   <xsl:template match="span">
      <fo:block font-size="12pt" line-height="15pt" space-after="12pt" space-before="-10pt">
         <xsl:apply-templates select="*|text()"/>
      </fo:block>
   </xsl:template>
  ...
  =============================================== -->
   ...
   <xsl:template match="pb">
    ...
    <!-- ===========================================
    A table for maps and legend. The map on the left
    and the legend on the right side.
    ============================================== -->
    <xsl:template match="map">
        <fo:table width="100%" height="100%" table-layout="fixed">
            <fo:table-column column-width="180mm"/>
            <fo:table-column column-width="60mm"/>
            <fo:table-column column-width="proportional-column-width(1)"/>
            <fo:table-body>
                <fo:table-row>
                  <fo:table-cell display-align="left" column-number="1">
                    <fo:block text-align="left">
                           <xsl:apply-templates select="mapimg"/>
                       </fo:block>
                   </fo:table-cell>
                <fo:table-cell column-number="2">
                       <fo:block>
                           <xsl:apply-templates select="maplegend"/>
                       </fo:block>
                   </fo:table-cell>
                </fo:table-row>
            </fo:table-body>
    ...
    <!-- ================================================
    Image template for maps. Uses scale-to-fit to get the
    optimal size.
    ================================================== -->
    <xsl:template match="mapimg">
       <fo:external-graphic content-width="scale-to-fit" width="170mm" content-height="scale-to-fit" height="150mm" vertical-align="top" src="{@src}"/>
    </xsl:template>
    <xsl:template match="maplegendimg">
       <fo:external-graphic content-width="scale-to-fit" width="5mm" content-height="scale-to-fit" height="5mm" vertical-align="middle" src="{@src}"/>
    </xsl:template>

    <xsl:template match="maplegend">
        <fo:block>
    ...
    <xsl:template match="maplegendheading">
        <fo:table-row>
            <fo:table-cell
                    font-size="12pt"
                    font-weight="bold"
    ...
    <!-- =============================================
    Map legend. A table with two columns. The left column
    has no contents but a background color (the legend)
    and on the right side is the text.
    ================================================ -->
   <xsl:template match="maplegendtable">
      <fo:table border-collapse="collapse" table-layout="fixed" height="100%"
              margin-top="15pt" margin-bottom="15pt" margin-left="15pt" margin-right="15pt">
              <xsl:attribute name="width">
            <xsl:choose>
              <xsl:when test="@width">
                  <xsl:value-of select="@width"/>
    ...
    <!-- ==========================================
    A row in the legend table.
    ============================================= -->
    <xsl:template match="legendrow">
        <fo:table-row>
            <xsl:apply-templates select="*|text()"/>
        </fo:table-row>
    </xsl:template>
    ...
    <!-- ==========================================
    A cell in the legend table.
    ============================================= -->
    <xsl:template match="legendcell">
        <fo:table-cell
                padding-start="0pt" padding-end="2pt" padding-before="2pt" padding-after="2pt"
                display-align="left" font-size="12pt" margin-left="0pt">
  ...
  =============================================== -->
   ...
   <xsl:template match="table">
      <fo:table table-layout="fixed" width="100%"> <!-- space-after="0pt"> -->
   ...
   <xsl:template match="markings">
      <fo:table space-after="12pt" font-size="6pt" table-layout="fixed">
         <fo:table-column column-width="20pt"/>
   ...
   <xsl:template match="caption">
      <fo:block space-before="10pt" font-size="6pt" font-weight="bold">
         <xsl:apply-templates select="*|text()"/>
      </fo:block>
   </xsl:template>
  ...
  =============================================== -->
   ...
   <xsl:template match="td">
      <fo:table-cell
         ...
         padding-before="2pt" padding-after="2pt" width="auto" height="auto">
         <xsl:if test="@background-color">
            <xsl:attribute name="background-color">
               <xsl:value-of select="@background-color"/>
            </xsl:attribute>
  ...
  =============================================== -->
   ...
   <xsl:template match="thead">
    ...
    <!-- ===========================================
    This table header is for repeating table headers
    when a table spans several pages.
    ============================================ -->
  ...
  =============================================== -->
   ...
   <xsl:template match="metadata">
      <fo:block page-break-before="always" space-after="2pt">
          <xsl:apply-templates select="*"/>
      </fo:block>
   </xsl:template>
         ...
         <xsl:choose>
            ...
            <xsl:when test="number($next-col) &gt; 0">
               ...
               <fo:table-column column-width="
                    if (ends-with($next-col,'pt')) then concat(25.4*$next-col div 72, 'mm')
                    if (ends-with($next-col,'mm')) then concat($next-col, 'mm')
